...
resource "aws_route_table_association" "web" {  #Used for mapping of newly created route tables
  count          = length(var.web_subnets_cidr)
...
resource "aws_route_table_association" "app" {  #Used for mapping of newly created route tables
  count          = length(var.app_subnets_cidr)
...
resource "aws_route_table_association" "db" {  #Used for mapping of newly created route tables
  count          = length(var.db_subnets_cidr)
...
#  subnet_id     = aws_subnet.web[0].id #lookup(element(aws_subnet.main,0),"id",null) this also can be used
